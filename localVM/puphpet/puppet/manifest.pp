...
if $composer_values == undef {
  $composer_values = hiera('composer', false)
} if $php_values == undef {
  $php_values = hiera('php', false)
} if $apache_values == undef {
  $apache_values = hiera('apache', false)
...
if hash_key_equals($composer_values, 'install', 1) {
  create_resources( 'class', { 'composer' => $composer_values['settings'] })
}
